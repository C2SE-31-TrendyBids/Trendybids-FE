...
const TrendingAuction = () => {
    ...
    useEffect(() => {
        const fetchAPI = async () => {
            ...
            responseCensor?.status === 200 &&
                setAuctionSession(responseCensor?.data?.productAuctions);
            responseCensor?.status === 200 &&
                setFirstSessions(responseCensor?.data?.productAuctions[0]);
        };
        fetchAPI();
    }, []);
    ...
    useEffect(() => {
        ...
        if (!sessionStorage.getItem(`hasCalledAPI_${firstSessions?.id}`)) {
            ...
            const startDate = moment(firstSessions?.startTime).format(
                "YYYY-MM-DD HH:mm:ss"
            );
            const endDate = moment(firstSessions?.endTime).format(
                "YYYY-MM-DD HH:mm:ss"
            );
            let timeoutId;
            ...
            if (currentDate >= startDate && currentDate < endDate) {
                timeoutId = setTimeout(async () => {
                    ...
                    setFirstSessions((prevState) => ({
                        ...prevState,
                        status: "ongoing",
                    }));
                    sessionStorage.setItem(
                        `hasCalledAPI_${firstSessions?.id}`,
                        "true"
                    ); // Set the flag after calling the API
                }, delay);
    ...
    return (
        ...
        >
            <div className="px-10 mx-auto mt-20 mb-4">
                <h2 className="text-[#0B1133] text-2xl font-bold">
                    Trending Auctions
                </h2>
                <p className="text-[#5F5F5F] text-xm font-thin">
                    ...
                    <div className="w-full h-full grid grid-rows-6">
                        <div className="relative row-span-6 rounded-t-lg group transition-all overflow-y-hidden overflow-x-hidden min-h-[400px]">
                            <img
                                ...
                                src={
                                    firstSessions?.product?.prdImages?.[0]
                                        ?.prdImageURL || ""
                                }
                            ...
                            {firstSessions?.status === "not_started" && (
                                <CountdownTimer
                                    targetDate={firstSessions?.startTime}
                                />
                            )}
                            {/*auction session starting*/}
                            {firstSessions?.status === "ongoing" && (
                                <p className="absolute left-2 top-2 bg-green-400 text-xm rounded px-4 py-1  text-white font-normal">
                                    ongoing
                                </p>
                            )}
                            {/*auction session ended*/}
                            {firstSessions?.status === "ended" && (
                                <p className="absolute left-2 top-2 bg-red-500 text-xm rounded px-4 py-1 text-white font-normal">
                                    ended
                                </p>
                            )}
                        ...
                        <div className="row-span-2 h-full bg-gray-200 ">
                            <div className="text-left text-[#0B1133] px-2 pt-3">
                                <p className="font-semibold text-xs">
                                    {firstSessions?.product?.category?.name ||
                                        ""}
                                </p>
                                <h5 className="text-lg font-bold">
                                    {firstSessions?.product?.productName || ""}
                                </h5>
                            </div>
                            <div className="text-right text-[#0B1133] px-2 pb-3">
                                ...
                                <h5 className="text-lg font-bold">
                                    $
                                    {firstSessions?.product?.startingPrice ||
                                        ""}
                                </h5>
                            </div>
                        </div>
