...
const ManagementPost = () => {
    ...
    const [order, setOrder] = useState({ id: "ASC", price: "ASC", status: "ASC" });
    ...
    return (
        <div className="">
            ...
            {showForm && (
                <div
                    className="fixed top-0 bottom-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
                    <div className="max-h-[95%] w-[50%] bg-white p-6 rounded-md shadow-lg scroll-smooth overflow-auto">
                        ...
                        >
                            <div className="grid sm:grid-cols-2 gap-8 overflow-y-auto py-2">
                                {values.status !== "" && (
                                    <div
                                        className={`sm:col-span-2 text-center py-2 px-2 font-semibold border-2 rounded text-xs ${values.status === "Processing"
                  ...
                  border-gray-100 focus:border-blue-500 rounded outline-none"
                            ...
                            <div className="relative flex items-center border border-gray-200 rounded-md mt-4">
                                <label
                                    className="text-[13px] bg-white text-black absolute mt-4 px-2 top-[-10px] left-[18px] font-semibold">
                                    Product Image
                                </label>
                                <div className="container">
                                    ...
                                    >
                                        <input {...getInputProps()} />
                                        <FaCloudUploadAlt
                                            className={`mx-auto w-10 h-10 text-blue-500 ${images.length > 0 ? "" : ""
                                                }`}
                                        />
                                        <p
                                            className={`text-center text-sm mt-4 text-gray-400 ${images.length > 0
                                                ? "hidden"
                                                : ""
                                                }`}
                                        >
                                        ...
                                        <p
                                            className={`text-center ${images.length > 0
                                                ? "hidden"
                                                : ""
                                                }`}
                                        >
                            ...
                            <div className="flex items-center justify-between mt-4">
                                <div className="flex items-center">
                                    ...
                                    <span>You must pay 20.00 USD to post the product to the auction organization</span>
                                </div>
                                {statusPayment || censorPaymentId === '' ? (
                                ...
                                {
                                    openPayment && <ModalPay modalOpen={setOpenPayment} amount={20} accessToken={token} setStatus={setStatusPayment} status={statusPayment} index={2} receiverId={censorPaymentId} auctionId={null} />
                                }
                            ...
                            <button
                                type="submit"
                                className={`mt-8 px-6 py-2.5 w-full text-sm font-semibold bg-blue-500 text-white rounded hover:bg-blue-600 relative ${loading
                            ...
                            >
                                {loading ? (
                                    <div className="flex items-center justify-center">
                                        <span>Loading...</span>
                                        <div
                                            className="animate-spin absolute top-1/2 -translate-y-1/2 right-4 h-5 w-5 border-t-2 border-b-2 border-white rounded-full"></div>
                                    </div>
                                ) : (
            ...
            <table className="w-full bg-white">
                <thead className="rounded-lg">
                    <tr className="grid grid-cols-12 gap-2 place-items-center bg-[#F3F4F6] rounded">
                        <th className="flex items-center gap-1 col-span-1 px-4 py-3 text-left text-md font-semibold hover:text-[#007bff] cursor-pointer text-gray-700 transition-all"
                            onClick={() => {
                                const sortDir = order.id === "ASC" ? "DESC" : "ASC";
                                setOrder(prevState => ({ ...prevState, id: sortDir }))
                                setParams(prevState => ({ ...prevState, order: ["id", sortDir] }))
                            }}
                        >
                            ID
                            <TiArrowUnsorted />
                        ...
                        <th className="flex items-center gap-1 col-span-2 px-4 py-3 text-left text-md font-semibold hover:text-[#007bff] cursor-pointer text-gray-700 transition-all"
                            onClick={() => {
                                const sortDir = order.price === "ASC" ? "DESC" : "ASC";
                                setOrder(prevState => ({ ...prevState, price: sortDir }))
                                setParams(prevState => ({ ...prevState, order: ["startingPrice", sortDir] }))
                        ...
                        <th className="flex items-center gap-1 col-span-2 px-4 py-3 text-left text-md font-semibold hover:text-[#007bff] cursor-pointer text-gray-700 transition-all"
                            onClick={() => {
                                const sortDir = order.status === "ASC" ? "DESC" : "ASC";
                                setOrder(prevState => ({ ...prevState, status: sortDir }))
                                setParams(prevState => ({ ...prevState, order: ["status", sortDir] }))
                        ...
                        products.length > 0 ?
                            products.map((prod, index) => (
                                <Tooltip title={prod?.note !== null ? `Reason: ${prod?.note}` : ""}
                                    ...
                                    <tr className="grid grid-cols-12 gap-2 place-items-center hover:bg-gray-50" key={index}>
                                        <td className="col-span-1 px-4 py-3 ">#{prod?.id.substring(0, 4)}</td>
                                        ...
                                        <td className="col-span-2 px-4 py-3 ">
                                            <span
                                                className={`w-[68px] block text-center py-0.5 font-semibold rounded text-xs ${prod.status === "Processing"
                            ...
                            <div className="flex justify-center items-center text-center mt-20">
                                <img src={emptyProducts} className="w-60 h-60" alt="Empty Products" />
                            </div>
                    }
