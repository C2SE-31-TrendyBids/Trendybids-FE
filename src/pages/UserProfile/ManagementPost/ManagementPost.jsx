...
import { getAllCensor } from "./../../../services/censor";
...
const ManagementPost = () => {
  ...
  const getData = async (accessToken, { productName = "", page = 1 }) => {
    if (productName) {
      page = 1;
    }

    const [response] = await Promise.all([getAllProduct(accessToken, { productName, page })]);

    if (response.response) {
      setProducts(response?.response?.products);
      setPagination((prev) => ({ ...prev, totalPage: response?.response?.totalPages }));
    }
  };

  useEffect(() => {
    (async () => {
      const [responseCate, responseCentos] = await Promise.all([getAllCategory(), getAllCensor()]);

      setCategories(responseCate?.response?.categorys || []);
      setCentos(responseCentos?.response?.censors || []);
      console.log({ responseCate, responseCentos });
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    getData(accessToken, { productName: debouncedSearchTerm, page: pagination.page });
  }, [token, debouncedSearchTerm, pagination.page]);
  ...
  const handleSubmit = async (event) => {
    ...
    const { response, error } = await (data?.id
      ? updateProduct({ accessToken, data })
      : addProduct({ accessToken, data }));
    ...
    if (response) {
      toast.success(
        data?.id ? `Product Update Successfully` : "Add Product Successfully"
      );
      setShowForm(false);
      setValues({
        productName: "",
      ...
      setImages([]);
      await getData(accessToken, { page: pagination.page, productName: "" });
    }
  };
  ...
  const handleDeleteProduct = async (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const { response, error } = await deleteProduct({ accessToken, id });
  ...
  return (
    <div className="overflow-x-auto ">
      <div className="flex items-center justify-between my-10">
        <h1 className="text-[20px] font-extrabold text-[#007bff]">Manage Post Auction Products</h1>
      ...
      {showForm && (
        <div className="fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-md shadow-lg scroll-smooth overflow-auto h-full">
            <div className="flex items-center justify-between mb-8">
              <h1 className="text-xl font-bold text-[#007bff]  ">Post Product Auction</h1>
              <button
                onClick={onClose}
            ...
            <form
              ...
              className="font-[sans-serif] m-6 max-w-6xl mx-auto w-[700px]"
            >
              ...
              <div className="relative flex items-center border border-gray-200 rounded-md mt-4">
                ...
                <div className="container">
                  <div {...getRootProps()} className="dropzone mt-10 ">
                    <input {...getInputProps()} />
                    <FaCloudUploadAlt
                    ...
                    <p className={`text-center ${images.length > 0 ? "hidden" : ""}`}>
                      Drag and drop files to upload
                    </p>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    {images.map((file, index) => (
                      <div key={index} className="image-preview flex flex-col items-center">
                        <img
                          src={file?.prdImageURL || URL.createObjectURL(file)}
                          alt="Preview"
                          className="w-20 h-20 mb-2"
                        />
                        <div className="flex items-center">
                          <p className="">{file.name}</p> {/* Display file name */}
                          <button className="remove-btn ml-2" onClick={() => removeImage(index)}>
                            <FaTimes />
                          </button>
                        </div>
                      </div>
              ...
              <button
                type="submit"
                className="mt-8 px-6 py-2.5 w-full text-sm font-semibold bg-blue-500 text-white rounded hover:bg-blue-600"
              >
                Submit
              </button>
            </form>
        ...
        <tbody className="whitespace-nowrap">
          {products.map((prod, index) => (
            <tr className="hover:bg-gray-50" key={index}>
              ...
              <td className="px-6 py-4 text-base truncate max-w-4">{prod.productName}</td>
              ...
              <td className="px-6 py-4 text-base truncate max-w-4">{prod?.description}</td>
              <td className="px-6 py-4 text-base">{prod?.startingPrice}</td>
              ...
              <td className="px-6 py-3">
                <button className="mr-4" title="Edit" onClick={() => handleOpenEdit(prod)}>
                  <svg
          ...
          <ul className="flex space-x-1 ml-2">
            ...
            <li
              onClick={() =>
                setPagination((prev) => ({
                  ...prev,
                  page: prev.page + 1 >= prev.totalPage ? prev.totalPage : prev.page + 1,
                }))
              }
              className="flex items-center justify-center cursor-pointer bg-gray-300 w-7 h-7 rounded"
            >
